...
Common::String windowTypeName(byte windowType) {
	int i = 0;
	while (g_borderNames[i].type != 0xFF) {
...
void MacWindowManager::loadDataBundle() {
	...
	if (!_dataBundle) {
		error("ENGINE: Couldn't load data bundle '%s'.", MACGUI_DATA_BUNDLE.c_str());
	}
}
...
Common::Rect MacWindowManager::getBorderOffsets(byte windowType) {
	...
	while (g_borderNames[i].type != 0xFF) {
		if (g_borderNames[i].type == windowType) {
			Common::Rect offsets;
			offsets.left = g_borderNames[i].lo;
			offsets.right = g_borderNames[i].ro;
			offsets.top = g_borderNames[i].to;
			offsets.bottom = g_borderNames[i].bo;
			return offsets;
		}
		i++;
	}
